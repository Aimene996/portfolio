...
## GetMyPortfolio
You can execute the `GetMyPortfolio` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetMyPortfolio(dc: DataConnect, options?: useDataConnectQueryOptions<GetMyPortfolioData>): UseDataConnectQueryResult<GetMyPortfolioData, undefined>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetMyPortfolio(options?: useDataConnectQueryOptions<GetMyPortfolioData>): UseDataConnectQueryResult<GetMyPortfolioData, undefined>;
```

### Variables
The `GetMyPortfolio` Query has no variables.
### Return Type
Recall that calling the `GetMyPortfolio` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetMyPortfolio` Query is of type `GetMyPortfolioData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetMyPortfolio`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig } from '@dataconnect/generated';
import { useGetMyPortfolio } from '@dataconnect/generated/react'

export default function GetMyPortfolioComponent() {
  ...
  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetMyPortfolio(dataConnect);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetMyPortfolio(options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetMyPortfolio(dataConnect, options);
...
You can execute the `ListFeaturedProjects` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useListFeaturedProjects(dc: DataConnect, options?: useDataConnectQueryOptions<ListFeaturedProjectsData>): UseDataConnectQueryResult<ListFeaturedProjectsData, undefined>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListFeaturedProjects(options?: useDataConnectQueryOptions<ListFeaturedProjectsData>): UseDataConnectQueryResult<ListFeaturedProjectsData, undefined>;
```

### Variables
The `ListFeaturedProjects` Query has no variables.
### Return Type
Recall that calling the `ListFeaturedProjects` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListFeaturedProjects` Query is of type `ListFeaturedProjectsData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListFeaturedProjects`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig } from '@dataconnect/generated';
import { useListFeaturedProjects } from '@dataconnect/generated/react'

export default function ListFeaturedProjectsComponent() {
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useListFeaturedProjects();

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListFeaturedProjects(dataConnect);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useListFeaturedProjects(options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useListFeaturedProjects(dataConnect, options);
...
export default function CreateNewProjectComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ portfolioId: ..., projectName: ..., description: ..., });
